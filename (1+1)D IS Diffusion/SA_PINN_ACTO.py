...
class PINN_IS_1D(nn.Module):
    def __init__(self, Nl, Nn, lb, ub):
        ...
        self.register_buffer('sscriptJ', torch.tensor(1.0, dtype=torch.get_default_dtype()))
        ...
        self.net.add_module('Linear_layer_final', nn.Linear(Nn, 2))  # [scriptJ_raw, alpha_raw]

        # IC algebraic enforcement toggles
        self.scriptJ_ic_func = None
    ...
    def forward(self, X_phys):
        ...
        scriptJ_raw,   alpha_raw   = raw[:, 0:1],   raw[:, 1:2]
        scriptJ_raw_L, alpha_raw_L = raw_L[:, 0:1], raw_L[:, 1:2]
        scriptJ_raw_R, alpha_raw_R = raw_R[:, 0:1], raw_R[:, 1:2]
    
        if self.scriptJ_ic_func is None or self.alpha_ic_func is None:
            raise RuntimeError("IC functions must be set before forward pass.")
    
        # Enforce periodic boundary conditions on raw outputs
        xi = (x_phys + L) / (2.0 * L)
        scriptJ_raw_periodic    = scriptJ_raw    - (scriptJ_raw_R    - scriptJ_raw_L)    * xi
        alpha_raw_periodic = alpha_raw - (alpha_raw_R - alpha_raw_L) * xi
        
        # Enforce initial condition
        scriptJ_ic_x    = self.scriptJ_ic_func(x_phys)
        alpha_ic_x = self.alpha_ic_func(x_phys)
    
        g = torch.exp(-1.0 * t_phys)  # g(0)=1
        h = 1.0 - g                   # h(0)=0
    
        scriptJ_enf = g * scriptJ_ic_x + h * scriptJ_raw_periodic
        alpha_enf = g * alpha_ic_x + h * alpha_raw_periodic
    
        # Scale back to physical units
        scriptJ_phys = self.sscriptJ * scriptJ_enf
        alpha_phys = self.sA * alpha_enf
    
        return torch.cat([scriptJ_phys, alpha_phys], dim=1)
    ...
    def pde_residual(self, x):
        ...
        out   = self.forward(x)            # [scriptJ, alpha]
        scriptJ    = out[:, 0:1]
        alpha = out[:, 1:2]
        ...
        scriptJ_t = self.gradients(scriptJ, x)[:, 0:1]

        N_0 = N_0_func(lambd, sigma, T, scriptJ, n, N_x, v)
        scriptJ_x = self.gradients(scriptJ, x)[:, 1:2]

        tauJ = tauJ_func(alpha, T)
        tauJ_sigmaT_t = self.gradients(tauJ/(sigma*T), x)[:, 1:2]

        R1 = n_t + scriptJ_x
        R2 = tauJ * scriptJ_t + scriptJ + 1/2 * sigma * T * scriptJ * tauJ_sigmaT_t + sigma * T * alpha_x

        sn = n_from_alpha_func(self.sA, T)
        return torch.cat([R1/sn, R2/self.sscriptJ], dim=1)
    ...
    def loss_mass(self, t_grid, x_grid):
        ...
        scriptJ_pred = out[:, 0].view(Nt, Nx)
        dx = (x_grid[1] - x_grid[0]).item()
        mass = scriptJ_pred.sum(dim=1) * dx
        mass0 = mass[0]
